...
function TamrielOMeter:RegisterRootMap(mapIndex)
    logger:Debug("注册根映射", self.adapter:GetFormattedMapName(mapIndex))
...
function TamrielOMeter:FindRootMapMeasurementForCoordinates(x, y)
    logger:Debug("为坐标查找根映射测量(%f, %f)", x, y)
    for rootMapIndex, measurement in pairs(self.rootMaps) do
        ...
        if(measurement and measurement:Contains(x, y)) then
            logger:Debug("坐标在里面 " .. self.adapter:GetFormattedMapName(rootMapIndex))
            return measurement
        end
    end
    logger:Warn("找不到与坐标匹配的根映射 (%f, %f)", x, y)
end
